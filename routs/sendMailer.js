    ...
    switch (lang) {
        ...
        case '日本語':
            const mailSentJP = await transporter.sendMail({
                ...
                to: ['kedinhofavorito@gmail.com', 'kledison2009@hotmail.com', email],
                subject: '歓迎します!',
                text: `様今回 ${name} に入会していただきありがとうございます。
                申し込み申請書をご確認ください。`,
                attachments: [
                    {
                        path: path
    ...
    switch (lang2) {
        case 'PT':
            const mailManegerPT = await transporter.sendMail({
                from: '"kledisom" <signatureprojectjp@gmail.com>',
                to: ['kedinhofavorito@gmail.com', 'kledison2009@hotmail.com', email2],
        ...
        case 'EN':
            const mailManegerEN = await transporter.sendMail({
                ...
                Attached is the registration form for the new student ${name}.
